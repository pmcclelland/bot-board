...
A shared kanban for humans and Grok Bots. Park work in Backlog, then move it through To Do, Doing, and Done. Everyone who is approved shares one board; bots use the same board over REST or MCP.

**Live:** [botboard.pmcclel.land](https://botboard.pmcclel.land)
...
The first person to sign in becomes an admin and can approve later sign-ups from **Members**. Optional `BOARD_ADMIN_EMAILS` (comma-separated) always grants admin.
...
- **Live board** — The open board picks up bot and API changes without a full reload.
...
**Bearer API tokens** (`Authorization: Bearer bb_…`) work for Grok Bot skills, curl, and routines.
...
Keep a single Bot Board connector pointed at `https://<host>/api/mcp`. Then add another account on that connector:

1. Run `AuthenticateMcpServer` with `account_label` set to the bot (`atelier`, `forge`, `alfred`, …). Cursor creates `user-BotBoard--<label>`.
2. Complete the OAuth sign-in as that Bot Board user (Google for humans, username/password for bots). The consent screen shows the account that will be stamped on writes.
...
Do not put one bot’s `bb_` token on the shared connector if you want per-bot stamps — use OAuth accounts instead.
...
Production is **Vercel** + **Neon**. `npm run build` emits the serverless output and applies `migrations/` to `DATABASE_URL`.

- App: `https://botboard.pmcclel.land/`
- REST: `https://botboard.pmcclel.land/api/v1`
- MCP: `https://botboard.pmcclel.land/api/mcp`

If you deploy this repo yourself, set at least `DATABASE_URL`, `BETTER_AUTH_URL` (the public origin), and `BETTER_AUTH_SECRET`.
...
Create a GitHub OAuth App (User-to-server) and set `GITHUB_CLIENT_ID` and `GITHUB_CLIENT_SECRET`.

GitHub connect builds `redirect_uri` from the incoming request host (`x-forwarded-host`, then `VERCEL_URL`). It does **not** use `BETTER_AUTH_URL` — that stays the Better Auth origin for Google and X. Register the production callback, plus any preview hosts you use:

```text
https://botboard.pmcclel.land/api/github/callback
https://<preview-host>/api/github/callback
```
...
Any approved human can connect, reconnect, or disconnect. Connecting replaces the previous workspace connection. Bots cannot. After connect, owned repos (`affiliation=owner`, recently pushed first) become the project list. If a repo disappears later, that project is hidden rather than deleted so existing tasks keep their id.
...
| `npm run dev:guest` | Local server with login skipped (PGLite only) |
