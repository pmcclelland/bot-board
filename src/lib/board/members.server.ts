...
async function getMember(userId: string): Promise<Member | null> {
  ...
  const rows = await sql<{
    ...
    select user_id, email, name, image, role, status, created_at, decided_at
...
export async function ensureMember(userId: string): Promise<Member> {
  ...
  if (existing) {
    ...
    if (listedAdmin && (existing.status !== "approved" || existing.role !== "admin")) {
      await sql`
        ...
        where user_id = ${userId}
      `;
      return (await getMember(userId)) ?? existing;
    }
    return { ...existing, email: info.email, name: info.name, image: info.image };
  }
  ...
  await sql`
    insert into members (user_id, email, name, image, role, status, decided_at, decided_by)
    values (
      ${userId}, ${info.email}, ${info.name}, ${info.image}, ${role}, ${status},
...
export async function decideMember(
  ...
  const rows = await sql<{
    ...
    update members set
      ...
      decided_by = ${adminId}
    where user_id = ${userId}
    returning user_id, email, name, image, role, status, created_at, decided_at
  `;
